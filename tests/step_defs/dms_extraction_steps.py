...
def ingest_test_data_into_rds(restore_sql, tasks_sql):
    ...
    count = 0
    backup_task_status = rds_connection.read_query(tasks_sql.format(task_id=task_id))[
        0
    ][5]
    while True:
        ...
        if count >= 15:
            raise TimeoutError(
                f"Failed to restore backup within adequate time: {count * 5}(s)"
            )

        time.sleep(15)
        count += 1

        backup_task_status = rds_connection.read_query(
            tasks_sql.format(task_id=task_id)
        )[0][5]
        ...
        logging.info(
            f"RDS still performing backup - current status {backup_task_status}"
        )
